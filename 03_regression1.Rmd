...
## или .csv
# download(
#   url = "https://varmara.github.io/mathmethr/data/nelson.csv", 
...
```{r}
library(readxl)
nelson <- read_excel("data/nelson.xlsx", sheet = 1)
## или из .csv 
# nelson <- read.table(file="data/nelson.csv", header = TRUE, sep = "\t", dec = ".") 
...
- Величина потери веса мучных хрущаков коррелирует с относительной влажностью воздуха ($r = `r round(p_cor$estimate, 2)`$, $p `r format.pval(p_cor$p.value, eps = 0.01)`$)
...
![Интерпретация коэффициентов регрессии](images/interpretation-of-regression-coefficients.png "Интерпретация коэффициентов регрессии")
...
```{r, nelson-conf, echo=FALSE, eval=FALSE}
```
...
```{r echo=FALSE, fig.height=3, out.height='3in'}
gg_nelson + 
  geom_smooth(method = "lm", 
              aes(fill = "Доверительный \nинтервал"), 
              alpha = 0.4) +
...
### Используйте $R^2 _{adjusted}$ для сравнения моделей с разным числом параметров
