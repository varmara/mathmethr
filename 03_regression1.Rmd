...
Как зависит плодовитость гибридов [смолевок](http://ru.wikipedia.org/wiki/Смолёвка) _Silene vulgaris_ от генетической удаленности? 
...
\includegraphics[width=0.9\linewidth]{images/BladderCampion.jpg}
...
__Доверительный интервал коэффициента__ --- это зона, в которой с заданной вероятностью содержится среднее значение оценки коэффициента. Если $\alpha = 0.05$, то получается 95\% доверительный интервал.


$$b _1 \pm t _{\alpha, df = n - 2} \cdot SE _{b _1}$$

__Доверительная зона регрессии__ --- это зона, в которой с заданной вероятностью лежит регрессионная прямая при повторных выборках из генеральной совокупности. 
...
Формат формулы: `зависимая_переменная ~ независимые_переменные`
...
- Доля стерильной пыльцы у межвидовых гибридов значимо возрастает с увеличением генетического расстоянияя ($b _1 = `r  round(coef(hybrid_lm[1])[1], 2)`$, $t = `r round(summary(hybrid_lm)$coefficients[2, 3], 2)`$, $p < 0.01$)
...
```{r echo=FALSE, purl=FALSE}
smr_f <- summary(hybrid_lm)$fstatistic[3]
...
gg_hybrid + geom_smooth(method = 'lm', level = 0.99) + 
...
- В оценке коэффициентов регрессии ($b_0$ и $b_1$) и предсказанных значений ($\hat y_i$) существует неопределенность. Доверительные интервалы можно расчитать, зная стандартные ошибки.
