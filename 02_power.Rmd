...
gg_beta
```
__Ошибка II рода --- вероятность принять $H_0$, когда верна $H_A$__
...
__Мощность теста - вероятность отвергнуть $H_0$, когда верна $H_А$__
...
## A priori анализ мощности {.column-2}
...
## Боксплот корня из численности улиток (__sqlim100__) для двух сайтов<br />
...
>- Распределение стало больше походить на нормальное. Можно пользоваться t-критерием для сравнения значений корня из численности улиток (__sqlim100__) для двух сайтов.
...
```{r purl = FALSE}
pwr_snail <- pwr.t.test(n = NULL, d = effect_snail, power = 0.8, sig.level = 0.05, type = "two.sample", alternative = "two.sided")
pwr_snail
```
