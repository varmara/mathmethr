...
## Как оценить стандартное отклонение для расчета величины эффекта? {.build}
...
library(readxl)
minch <- read_excel(path = "minch.xls", sheet = 1)
```
...
## Что получилось бы с улитками из примера на самом деле?
...
## Проверим, различается ли плотность улиток между локациями

По умолчанию в R используется t-критерий для неравных дисперсий (Модификация Велча)
