...
p_raw <- ggplot(df) + geom_point(aes(x = x1, y = x2), colour = 'black') + geom_hline(yintercept = 0) + geom_vline(xintercept = 0) + geom_text(aes(x = x1, y = x2, label = rownames(df)), hjust = -0.3) 
df
```
...
p_normalized <- ggplot(dfnorm) + geom_point(aes(x = x1, y = x2), colour = 'black') + geom_hline(yintercept = 0) + geom_vline(xintercept = 0) + geom_text(aes(x = x1, y = x2, label = rownames(dfnorm)), hjust = -0.3)
```
...
```{r ggscor-pos, out.width='2in', out.height='2in', fig.align='center'}
...
```{r out.height='1.8in'}
library(cowplot)
plot_grid(ggloadings, ggscores, labels = c('A', 'B'), align = 'hv', axis = 'r')
```
...
```{r skull-ord, purl=FALSE, fig.width=4, fig.height=4,  out.height='1.8in'}
# Данные для графика ординации
df_scores1 <- data.frame(skulls, 
                         scores(ord1, display = 'sites', choices = c(1, 2, 3), 
                                scaling = 'sites'))
# График ординации
...
```{r out.height='2in'}
# library(cowplot)
plot_grid(ggloadings1, ggscores1, labels = c('A', 'B'), align = 'vh', axis = 'r')
