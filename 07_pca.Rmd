...
p_raw <- ggplot(as.data.frame(X)) + geom_point(aes(x = x1, y = x2), colour = 'black') + geom_text(aes(x = x1, y = x2, label = 1:nrow(X)), hjust = -0.3) + theme_classic() + coord_fixed(expand = FALSE, xlim = c(0, max(X[, 1]) + 1), ylim = c(0, max(X[, 2]) + 2))
head(X)
```
...
p_centered <- ggplot(as.data.frame(X_cent)) + geom_point(aes(x = x1, y = x2), colour = 'black') + geom_hline(yintercept = 0) + geom_vline(xintercept = 0) + geom_text_repel(aes(x = x1, y = x2, label = 1:nrow(X_cent)), hjust = -0.3) + coord_fixed()
```
